...
// Curried functions have been changed to return a closure instead, 
// conforming to https://github.com/apple/swift-evolution/blob/master/proposals/0002-remove-currying.md#detailed-design
...
func CIE2000SquaredColorDifference(
    ...
    return { (lab1:INVector3, lab2:INVector3) -> Float in
        ...
        let _T1 = 1 - 0.17 * cos(GLKMathDegreesToRadians(Hbp - 30))
        let _T2 = _T1 + 0.24 * cos(GLKMathDegreesToRadians(2 * Hbp))
        let _T3 = _T2 + 0.32 * cos(GLKMathDegreesToRadians(3 * Hbp + 6))
        let T = _T3 - 0.20 * cos(GLKMathDegreesToRadians(4 * Hbp - 63))
